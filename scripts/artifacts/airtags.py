...
def timestampcalc(timevalue):
    return datetime.datetime.fromtimestamp(int(timevalue) / 1000).strftime(
        '%Y-%m-%d %H:%M:%S'
    )
...
def get_airtags(files_found, report_folder, seeker, wrap_text):
    ...
    if data_list_crowdloc:
        ...
        tsvname = 'Airtgas Crowdsourced Locations'
        tsv(report_folder, data_headers_crowdloc, data_list_crowdloc, tsvname)

        tlactivity = 'Airtags Crowdsourced Locations'
